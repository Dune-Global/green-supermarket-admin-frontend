    ...
    return (
        <div>
            ...
            <div className='w-full relative lg:hidden'>
                <button
                    className="flex flex-col absolute right-0 top-[-5.25rem] items-center justify-center z-20 lg:hidden"
                    onClick={handleClick}
                >
                    <span
                        ...
                        <div className="fixed flex flex-col py-10 border-2 border-green-400 bg-gray-0 shadow-lg rounded-lg items-center justify-center top-0 left-0 right-0 z-10">
                            <div className='flex flex-col gap-9 py-3'>
                                {sideMenuItems.map((item) => (
                                    <div key={item.id}>
                                        <Link href={item.path} className={`flex gap-2 items-center ${pathName === item.path && "text-green-400"}`} >
                                            <item.icon size={22} strokeWidth={2} className='text-gray-200' />
                                            <h2>
                                                {item.description}
                                            </h2>
                                        </Link>
                ...
                <div className="flex flex-col">
                    {sideMenuItems.map((item) => (
                        <Link key={item.id} href={item.path} className='py-[1px]'>
                            <div className={`p-6 flex items-center gap-3 border-l-4 border-gray-0 hover:bg-gray-50 hover:border-gray-50 ${pathName === item.path && "bg-gray-50 !border-green-400"}`}>
                                <item.icon size={22} strokeWidth={2} className='text-gray-200' />
